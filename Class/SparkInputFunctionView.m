...
@interface SparkInputFunctionView () <UITextViewDelegate, AVAudioRecorderDelegate>
{
...
@property (nonatomic, strong) SparkInputToolsView *toolsView;


@end

@implementation SparkInputFunctionView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        WeakSelf(weakSelf);
        self.toolsView.selTypeBlock = ^(NSString * _Nonnull typeName) {
            if (weakSelf.selRooterBlock) {
                weakSelf.selRooterBlock(typeName);
            }
        };
        //发送消息
...
- (void)layoutSubviews
{
    ...
    self.toolsView.frame = CGRectMake(0, 0, self.uu_width, KeyBordToolsHight-35);
	self.btnVoiceRecord.frame = CGRectMake(70, KeyBordToolsHight-40, self.uu_width-70*2, 30);
	self.btnSendMessage.frame = CGRectMake(self.uu_width-40, KeyBordToolsHight-40, 30, 30);
	self.textViewInput.frame = CGRectMake(45, KeyBordToolsHight-40, self.uu_width-2*45, 30);
	_placeHold.frame = CGRectMake(20, 0, 200, 30);
	self.btnChangeVoiceState.frame = CGRectMake(5, KeyBordToolsHight-40, 30, 30);
}
...
- (void)cancelRecordVoice:(UIButton *)button
{
    ...
    [SparkChatProgressHUD dismissWithError:@"Cancel"];
}

- (void)RemindDragExit:(UIButton *)button
{
    [SparkChatProgressHUD changeSubTitle:@"Release to cancel"];
}

- (void)RemindDragEnter:(UIButton *)button
{
    [SparkChatProgressHUD changeSubTitle:@"Slide up to cancel"];
}
...
{
    [self.delegate SparkInputFunctionView:self sendVoice:voiceData time:playTime+1];
    [SparkChatProgressHUD dismissWithSuccess:@"Success"];
...
- (void)failRecord
{
    [SparkChatProgressHUD dismissWithSuccess:@"Too short"];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [self.superVC dismissViewControllerAnimated:YES completion:^{
        [self.delegate SparkInputFunctionView:self sendPicture:editImage];
    }];
}
