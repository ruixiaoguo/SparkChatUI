...
@interface SparkChatProgressHUD ()
{
    NSTimer *_myTimer;
    CGFloat _angle;
}
...
@implementation SparkChatProgressHUD
...
+ (instancetype)sharedView
{
    ...
    static SparkChatProgressHUD *sharedView;
    dispatch_once(&once, ^ {
        sharedView = [[SparkChatProgressHUD alloc] initWithFrame:[UIScreen uu_screenBounds]];
...
+ (void)show {
    [[SparkChatProgressHUD sharedView] show];
}

+ (void)changeSubTitle:(NSString *)str
{
	[SparkChatProgressHUD sharedView].subTitleLabel.text = str;
}

+ (void)dismissWithSuccess:(NSString *)str
{
	[[SparkChatProgressHUD sharedView] dismiss:str];
}
